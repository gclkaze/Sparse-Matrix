...
private:
  ...
  int findMaxIndex(const std::vector<bool> &zeroChildrenRemaining, const std::vector<int> nodePositions)
  {
    int max = 0;
    for (int i = 0; i < (int)zeroChildrenRemaining.size(); i++)
    {
  ...
  void cleanNodesAndChildren(const std::vector<int> &entries, const std::vector<int> &visitedNodes)
  {
    // clean flatChildren
